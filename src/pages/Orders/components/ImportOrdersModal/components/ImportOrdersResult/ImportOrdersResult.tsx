...
const ImportOrdersResult: React.FC<Props> = ({ data, onClose }) => {
  ...
  const successColumns = [
    successColumnHelper.accessor('order_source', {
      id: 'select-col',
      header: ({ table }) => (
        <Checkbox
    ...
    successColumnHelper.accessor('order_ref', {
      header: 'Order Ref',
      meta: 'slim',
      footer: (props) => props.column.id,
    }),
    ...
    successColumnHelper.accessor('order_SLA', {
      header: 'SLA',
      meta: 'slim',
      footer: (props) => props.column.id,
    }),
            ...
            <Box style={{ flexDirection: 'row', display: 'flex', gap: 10 }}>
              <Box flexGrow={1}>
                <Typography fontSize={12} fontWeight={400} color={COLORS.GREY}>
                  These orders have passed validation. Click import to create orders.
                </Typography>
              </Box>
              <Box>
