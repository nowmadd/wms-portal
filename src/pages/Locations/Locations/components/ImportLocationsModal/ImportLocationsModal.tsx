...
import { ImportCont, ImportLocationsCont, ModalBody, ModalFooter, UploadingItemsCont } from './ImportLocations.styles';
...
import ImportLocationsResult from './components/ImportLocationsResult/ImportLocationsResult';
...
const { verifyImportLocations } = locationsServices();

const ImportLocationsModal: React.FC<Props> = ({ onClose }) => {
  const [uploading, setuploading] = useState<string[]>([]);
  const [errorFiles, seterrorFiles] = useState<string[]>([]);
  const { mutateAsync, isLoading } = verifyImportLocations();
  ...
  const downloadCSV = () => {
    window.open(CONFIG.LOCATIONS_CSV_TEMPLATE_LINK, '_blank');
  };
  ...
  return isFinished && resultData ? (
    <ImportLocationsResult data={resultData} onClose={onClose} />
  ) : (
    <ImportLocationsCont>
      <ModalBody>
        <Button
          ...
          text={'Download Locations Template (10KB)'}
          color={'primary'}
        />
